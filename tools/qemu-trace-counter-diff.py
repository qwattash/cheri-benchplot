...
def main():
    ...
    parser.add_argument("--aggregate", action="store_true", default=False,
                        help="Aggregate the instruction count")
    ...
    base_df = all_df.filter(pl.col("name") == baseline).select(
        pl.col("fn"), cs.all().exclude("fn", "name").name.suffix("_baseline"))
    join_df = all_df.join(base_df, on="fn")

    diff_df = join_df.with_columns(
        *[(pl.col(c) - pl.col(f"{c}_baseline")).alias(f"{c}_diff") for c in data_cols]
    ).filter(pl.col("name") != baseline)

    if args.aggregate:
        diff_df = diff_df.group_by("name").sum()
    diff_df.write_csv(args.output.with_suffix(".csv"))
