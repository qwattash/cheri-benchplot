...
if [ -n "${OPT_RC_SETUP}" ]; then
    ...
    case "${ARCH}" in
        arm64)
            NEXT_KERNEL="GENERIC-MORELLO-NODEBUG"
            ;;
        *)
    ...
    arm64)
        case "${KERNEL}" in
            GENERIC-MORELLO-NODEBUG)
                WORKLOAD_SCRIPT="run-target-hybrid-kernel.sh"
                NEXT_KERNEL="GENERIC-MORELLO-PURECAP-BENCHMARK-NODEBUG"
                ;;
            GENERIC-MORELLO-PURECAP-BENCHMARK-NODEBUG)
                WORKLOAD_SCRIPT="run-target-benchmark-kernel.sh"
                ;;
            *)
                # Undexpected kernel
...
if [ -n "$WORKLOAD_SCRIPT" ]; then
    echo "Auto-run sequence (iperf.smoketest): workload ${WORKLOAD_SCRIPT}"
