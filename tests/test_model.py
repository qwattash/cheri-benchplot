...
expect = pd.DataFrame({
    ...
    "foo": [0xdead],
    "bar": [0xbeef]
}).set_index(["dataset_id", "dataset_gid", "param0", "param1", "foo"])
...
def test_dynamic_model_fields_extra_index(session_config, fake_session_factory):
    ...
    }).set_index(["dataset_id", "dataset_gid", "param0", "param1", "foo", "useless_index"])
...
def test_dynamic_model_fields_index_order(session_config, fake_session_factory):
    ...
    test_data = pd.DataFrame({
        ...
        "foo": [0xdead],
        "bar": [0xbeef]
    }).set_index(["dataset_id", "dataset_gid", "foo", "param0", "param1"])
