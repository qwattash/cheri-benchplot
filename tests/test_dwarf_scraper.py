...
def test_scraper_context(find_scraper, task_factory):
    ...
    assert task.script.context["dws_config"].dwarf_scraper == None
    assert task.script.context["dws_config"].verbose_scraper == False
    assert len(task.script.context["dws_config"].dwarf_data_sources) == 1
    assert task.script.context["dws_config"].dwarf_data_sources[0].path == Path(asset_file)
    assert task.script.context["dws_config"].dwarf_data_sources[0].matcher == None
    assert task.benchmark.get_benchmark_data_path() in task.script.context["dws_database"].parents
...
def test_scraper_load(find_scraper, task_factory, tmp_path):
    ...
    subprocess.run([find_scraper, "--database", database, "-i", asset, "flat-layout"], check=True)
