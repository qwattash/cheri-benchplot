...
class _QEMUHistTable(BenchmarkSubPlot, QEMUHistSubPlotMixin):
    """
    ...
    def _remap_display_columns(self, colmap: pd.DataFrame):
        ...
        show_cols = np.append(colmap.loc[:, common_cols].to_numpy().transpose().ravel(),
                              colmap.loc[colmap.index != baseline, rel_cols].to_numpy().transpose().ravel())
        return show_cols

    def _get_sort_by_columns(self):
...
class QEMUAllSymbolHistTable(_QEMUHistTable):
    """
...
class QEMUCommonSymbolHistTable(_QEMUHistTable):
    """
...
class QEMUExtraSymbolHistTable(_QEMUHistTable):
    """
    Plot a table with QEMU histogram statistics restricted to symbols hit in all
    benchmark runs.
    """
...
class QEMUStrangeSymbolHistTable(_QEMUHistTable):
    """
