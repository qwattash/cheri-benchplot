...
# from .model import (
#     ImpreciseSubobjectInfoModel,
#     ImpreciseSubobjectInfoModelRecord,
#     ImpreciseSubobjectLayoutModel,
# )
...
#         else:
#             self.logger.error(
#                 "DWARF TypeInfo extraction unsupported for %s",
#                 instance_config.cheri_target,
#             )
#             raise RuntimeError(
#                 f"Unsupported instance target {instance_config.cheri_target}"
#             )
#         return cap_format
...
#             (
#                 so.Plot(show_df, y="label", x="additional_precision", color="legend")
#                 .on(ax)
#                 .add(
#                     so.Bar(),
...
#             else:
#                 sns.ecdfplot(
#                     data=show_df, x="additional_precision", hue="legend", ax=ax
#                 )
...
#             return self._compute_platform_precision(
#                 r.name[gid_index],
#                 r.name[member_offset_index],
#                 r.name[member_offset_index] + r["member_size"],
#             )
...
#     def run_plot(self):
#         self._plot_precision(
#             self._prepare_dataset(),
#             self.imprecise_bits_plot,
#             self.imprecise_bits_cdf,
#             self.imprecise_abs_bits_cdf,
#         )
#         self._plot_precision(
#             self._prepare_dataset_filter_common(),
#             self.imprecise_common_bits_plot,
#             self.imprecise_common_bits_cdf,
#             self.imprecise_common_abs_bits_cdf,
#         )
...
#                 else:
#                     self.logger.warning(
#                         "Uncategorised sub-object node %s %s", node.member, attrs
#                     )
#                     continue
...
#                     dataset_gid=gid,
#                     category=category,
#                     alias_kind="alias-ptr"
#                     if attrs["alias_pointer_members"]
#                     else "other",
...
#             ax = fig.subplots()
#             count_df = (
#                 df.groupby(["platform", "category"], as_index=False)
#                 .size()
...
#             percent_df["% of all imprecise"] = (
#                 100 * percent_df["size"] / percent_df["total"]
#             )
#             sns.barplot(
#                 percent_df, x="category", y="% of all imprecise", hue="platform", ax=ax
#             )
...
#             count_df = df.groupby(["platform", "category"], as_index=False).size()
#             alias_ptr = (
#                 df[df["alias_kind"] == "alias-ptr"]
#                 .groupby(["platform", "category"], as_index=False)
#                 .size()
#             )
#             percent_df = alias_ptr.merge(
#                 count_df,
#                 on=["platform", "category"],
#                 suffixes=["_ptr", "_total_by_kind"],
#             )
#             percent_df["% aliasing with ptr"] = (
#                 100 * percent_df["size_ptr"] / percent_df["size_total_by_kind"]
#             )
#             sns.barplot(
#                 percent_df, x="category", y="% aliasing with ptr", hue="platform", ax=ax
#             )

#         # Plot distribution of imprecise sub-object sizes grouped by platform, with one faced for
#         # each aliasing kind.
#         with new_facet(
#             self.categories_sizes_per_alias_kind.paths(),
#             df,
#             row="platform",
#             height=5,
#             aspect=2.5,
...
#             facet.map_dataframe(
#                 sns.histplot, x="size", hue="category", bins=hist_bins, element="step"
#             )
