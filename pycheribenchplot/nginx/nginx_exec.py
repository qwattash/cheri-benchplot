...
class NginxScenario(Config):
    endpoint: str = config_field(MISSING, desc="HTTP(s) endpoint for the test")
    nginx_config: str = config_field(MISSING,
...
class NginxBenchConfig(TVRSExecConfig):
    prefix: Optional[ConfigPath] = config_field(Path("/usr/local/nginx"), desc="Configured --prefix of nginx")
    www_path: Optional[ConfigPath] = config_field(None, desc="Path to the www directory to import to the session")
...
class NginxBenchExec(TVRSExecTask):
    ...
    def run(self):
        ...
        if self.config.www_path:
            shutil.copytree(self.config.www_path, self.session.get_asset_root_path() / "www", dirs_exist_ok=True)
        ...
        if not (crt_path / "server.crt").exists():
            ...
            sp.run([
                "openssl", "req", "-nodes", "-new", "-x509", "-keyout", crt_path / "server.key", "-out",
                crt_path / "server.crt", "-subj", "/C=UK/O=test/CN=*.cheri-nginx-benchmark.local"
            ],
                   stdout=sp.DEVNULL,
                   check=True)
