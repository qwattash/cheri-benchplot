...
class C18nTransitionFrequencyPlot(SlicePlotTask):
    ...
    def run_plot(self):
        ...
        total_chunks = [
            df.group_by(self.param_columns_with_iter).len("total_transitions")
            for df in chunks
        ]
        ...
        tc_chunks = [
            df.group_by([*self.param_columns_with_iter, "caller", "callee"]).len(
                "transition_count"
            )
            for df in chunks
        ]
        ...
        df = (
            df.filter(pl.col("caller") == pl.col("callee"))
            .group_by(self.param_columns_with_iter)
            .agg(
        ...
        df = df.with_columns(
            (pl.col("total_transitions") - pl.col("self_transitions")).alias(
                "cross_transitions"
            ),
            (pl.col("self_transitions") / pl.col("total_transitions")).alias(
        ...
        df = df.unpivot(
            on=["total_transitions", "self_transitions", "cross_transitions"],
