...
template <typename CCPerms> struct CCExtraOps {};

template <> struct CCExtraOps<cc64_cap> {
...
template <> struct CCExtraOps<cc128_cap> {
  ...
  static void setAddr(cc128_cap &Cap,
                      typename CompressedCap128::addr_t Cursor) {
    cc128_set_addr(&Cap, Cursor);
  }
};

template <> struct CCExtraOps<cc128m_cap> {
  ...
  static void setAddr(cc128m_cap &Cap,
                      typename CompressedCap128m::addr_t Cursor) {
...
void defineCap(py::handle M, const char *Name) {
  ...
  auto PyCap =
      py::class_<CC>(M, Name)
          ...
          .def("flags", &CC::flags)
          .def("__eq__",
               [](const CC &Left, const CC &Right) { return Left == Right; })
          .def("__str__",
               [](const CC &Cap) {
                 std::stringstream SS;
          ...
          .def("__repr__",
               [](const CC &Cap) {
                 ...
                 SS << std::boolalpha
                    ...
                    << std::hex << "Perms: 0x" << Cap.permissions() << "\n"
          ...
          .def("compress_raw",
               [](const CC &Cap) { return CCOps::compress_raw(Cap); })
          .def("compress_mem",
               [](const CC &Cap) { return CCOps::compress_mem(Cap); })
          .def("setbounds",
               [](CC &Cap, LengthT Length) {
                 return CCOps::setbounds(&Cap, Length);
               })
          .def("setaddr",
               [](CC &Cap, AddrT Cursor) { CCExtra::setAddr(Cap, Cursor); })
          .def_static("make_max_perms_cap",
                      [](AddrT Base, AddrT Cursor, LengthT Top) {
                        return CCOps::make_max_perms_cap(Base, Cursor, Top);
                      })
          .def_static("make_max_bounds_cap",
                      [](AddrT Cursor) {
          ...
          .def_static(
              "make_null_derived_cap",
              [](AddrT Addr) { return CCOps::make_null_derived_cap(Addr); })
          .def_static("decompress_raw",
                      [](AddrT Pesbt, AddrT Cursor, bool Tag) {
                        return CCOps::decompress_raw(Pesbt, Cursor, Tag);
                      })
          .def_static("decompress_mem",
                      [](AddrT Pesbt, AddrT Cursor, bool Tag) {
                        return CCOps::decompress_mem(Pesbt, Cursor, Tag);
                      })
          .def_static(
              "representable_mask",
              [](AddrT Len) { return CCOps::representable_length(Len); })
          .def_static("representable_mask",
                      [](AddrT Len) { return CCOps::representable_mask(Len); })
          .def_static("get_mantissa_width",
                      []() { return CCExtra::MantissaWidth; });
  CCExtra::definePerms(PyCap);
}

PYBIND11_MODULE(_pychericap, M) {
