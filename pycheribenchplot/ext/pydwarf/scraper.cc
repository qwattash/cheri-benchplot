...
fs::path DwarfSource::GetPath() const {
  return path_;
}

llvm::DWARFContext& DwarfSource::GetContext() const {
  return *dictx_;
}
...
DwarfScraper::DwarfScraper(StorageManager &sm, std::shared_ptr<const DwarfSource> dwsrc)
    : sm_{sm}, dwsrc_{dwsrc} {
  InitSchema();
}
