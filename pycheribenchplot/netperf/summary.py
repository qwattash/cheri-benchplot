...
class NetperfPlotConfig(PlotGridConfig):
    netperf_metrics: List[str] = config_field(
        lambda: ["throughput", "mean latency microseconds"],
...
class NetperfSummaryPlot(TVRSParamsMixin, PlotTask):
    ...
    def run_plot(self):
        ...
        df = df.unpivot(
            index=index_cols,
            on=self.config.netperf_metrics,
        ...
        stats = self.compute_overhead(
            df, "value", extra_groupby=["metric"], how="median", overhead_scale=100
        )
        ...
        with PlotGrid(self.summary, median_df, grid_config) as grid:
            grid.map(
                grid_barplot, x="scenario", y="value", err=["value_low", "value_high"]
            )
            grid.add_legend()
